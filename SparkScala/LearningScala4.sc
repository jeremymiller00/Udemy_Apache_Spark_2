...
val captainStuff = ("Picard", "Enterprise-D", "NCC-1701-D")
                                                //> captainStuff  :(String, String,String) = (PicardEnterprise-DNCC-1701-D)
                                                //| 
println(captainStuff)                           //> (PicardEnterprise-D,NCC-1701-D)
...
// Lists
// Like a tuple, but it's an actual Collection object that has morefunctionality.
// Also, it cannot hold items of different types.
// It's a singly-linked list under the hood.

val shipList = List("Enterprise", "Defiant", "Voyager", "Deep SpaceNine")
...
// Let's apply a function literal to a list! map() can be used toapply any function to every item in a collection.
...
println(numbers.filter( (x:Int) => x % 3 == 0 ))
for (n <- numbers) {
  if (n % 3 == 0) {
    println(n)
  } 
